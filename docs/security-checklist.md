...
| 20 | A manager cannot reach Policy settings, the Managers screen, notifications or the full audit log by typing the URL — the request-boundary guard turns them away and each page re-checks against the profile row — and a manager suspended mid-session loses the admin area on their next request | ✅ | `tests/e2e/manager-access.spec.ts` |
