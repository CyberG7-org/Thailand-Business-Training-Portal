...
| 21 | The service-role steps a manager can reach — the vector store and signed recording URLs — come only after a read under the caller’s client, and upload URLs are signed by the caller’s own client under storage RLS, so another team’s record id yields nothing; object keys are constrained to `<own record id>/<file>` with no `..`, so a key a manager writes can never point the service role at another team’s file; and nothing that identifies a company travels with the shared question bank | ✅ | `tests/integration/team-gates.test.ts`, `tests/integration/team-review-fixes.test.ts`, `tests/integration/team-shared-content.test.ts` |
