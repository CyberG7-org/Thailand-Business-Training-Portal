...
1. **Batch generation** (`/admin/questions/generate`): questions depend on the DBD certificate. The admin picks a confirmed DBD record as the **reference** (its particulars and, when uploaded, its certificate PDF go to the model as an example), a count (all personalised by default), optional extra material (study cards, pasted text, PDF/DOCX/TXT/MD) for document-knowledge questions, pools and difficulty. One model call returns every question in **TH, EN and ZH at once**. Everything is inserted as `approval_status = 'draft'`, `source = 'ai_generated'`, grouped by a `question_generation_batches` row.
...
- **D36** Every run carries a PII-free description of the certificate structure (`dbd-reference.ts`); a reference record's literal values are banned from the output (validation rejects leaks), so shared questions stay personalised through placeholders only.
...
1. `generateQuestionsAction` → `generateQuestionsIntoBank(db, adminId, input)`: loads the reference record (+ certificate PDF from `dbd-documents`), builds the material bundle, calls `generator.generate`, validates (shape, placeholders, banned reference literals), inserts batch + questions (keys `ai-<batch8>-<n>`) + 3 localizations each (`upsertQuestionLocalization` recomputes kind/dependencies), returns `{ batchId, produced, rejected }`; redirect to `/admin/questions?batch=<id>`.
