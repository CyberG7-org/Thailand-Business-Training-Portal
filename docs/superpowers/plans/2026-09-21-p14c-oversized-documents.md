...
The reviewer returned "not ready" with three Critical and eight Important findings; all were fixed test-first in one pass on the branch (see the ledger for rulings). What changed against the tasks above:

- **C2/I6 — trigger.** `extractFromTranscripts` no longer runs inline from the cron hook or inside `extractAndApply`. It is `fillRecordFromTranscripts` (`lib/db/transcript-extraction.ts`), run by the worker for a new job kind `transcript` (migration 0017: `index_jobs.kind` gains `transcript`; new table `dbd_sweeps`). The worker queues that job itself when a document with `page_count > DIRECT_READ_MAX_PAGES` becomes ready (`onDocumentReady` is gone), and `extractAndApply` queues it (`enqueueTranscriptJob`) instead of reading transcripts — with `rereadFacts` when nothing was small enough to read directly. The action result gained `transcripts: 'queued' | 'pending_index' | 'none'`; the UI shows *queuedFill* / *deferredFill* accordingly, and "Read again" never shows a raw `deferred` code (I1).
- **C2/I8 — resumability.** Sweep batches (`TRANSCRIPT_SWEEP_PAGES`, default 10, clamp 1..100) are cached per document in `dbd_sweeps`; a run releases its job when the worker's budget is spent (at least one sweep per run) and the next run continues from the cache. A batch whose answer overflows `max_tokens` (now reported as `too_large` from the stream's stop reason; sweep ceiling raised to 32000) is re-read page by page. Re-indexing a document clears its cached sweeps.
- **C3 — writes.** Level 1/3 columns are written one conditional update each (`is null` / empty directors, `extraction_status <> 'confirmed'`); `structured_data` and `extraction_raw` are merged under an `updated_at` compare-and-swap; a record confirmed mid-run takes nothing. The particulars are read only when no extraction is stored (or on Read again) and are merged into `extraction_raw` so the form shows the confidence cue for transcript-filled fields (I4).
- **C1/I3 — lists.** `selectByTypeAuthority` takes each list from the document kind authoritative for it (`LIST_AUTHORITY`); `sanitizeSweptLists` drops rows the stored schema rejects and caps lists at `PROFILE_LIST_LIMITS` (objectives 1000, shareholders 5000, promoters 100 — raised from 500/500/100). Sweeps stop as soon as every list a kind can supply is filled.
- **I4 — direct pass.** `directPass` merges provenance and keeps the rest of `structured_data` (the pre-existing loss of Level 4 answers on every direct pass is fixed with it).
- **I5 — classification.** From the first two non-blank pages of the first slice; at completion a document still `null`/`other` is classified again from its first six pages.
- **I7 — no index.** With `VECTOR_PROVIDER=off` the direct read takes documents up to the model's 100 pages; beyond that the outcome is `too_large`, never a fill that would never come.
- Tests: `tests/unit/integrations/transcript-merge.test.ts`, streamed-call cases in `claude-streaming.test.ts`, the rewritten `tests/integration/transcript-extraction.test.ts` (resume from cache, conditional writes under a racing admin, page-by-page fallback, 600-row list, mixed pack through both passes), worker cases in `dbd-index.test.ts` (cover-sheet classification, transcript job dispatch/release/backoff), provider-off cases in `extraction.test.ts`, and the e2e now also drives "Read again" through the queue.
