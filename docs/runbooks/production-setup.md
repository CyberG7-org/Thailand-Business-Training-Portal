...
2. Environment variables (Production, and Preview if you run a staging copy):
   ...
   | `DIRECT_READ_MAX_PAGES` | optional, default 20 | documents with more pages are never sent whole to the model; they fill in from their transcripts once indexed (P14c) |
...
- Packs over 20 pages fill in after the index is ready (about a minute per 5 pages); the record page says so and the fields appear on reload. **Read the document again** re-runs both paths.
